...
5. ステートマシンの作成
  ...
  - ページ上側にある [**設定**] をクリック
  - **アクセス許可** セクション
    - **実行ロール** で **新しいロールを作成** を選択
  - ページ右上にある [**作成**] をクリック
  - [**ロールの作成を確認**] ダイアログが表示された場合は、[**確認**] をクリック
...
1. 作成したステートマシンのページで [**詳細**] セクションから、[**IAM ロール ARN**] のロールの ARN のリンクをクリックし、IAM のページからロールを削除します。
1. 作成したステートマシンのページで、[**アクション**] - [**ステートマシンの削除**] をクリックして、ステートマシンを削除します。
1. CloudFormation のページから AWS SAM で作成したスタックを削除します。
1. CloudWatch のページで、[**ログ**] - [**ログ管理**] を選択して、`/aws/lambda/sfn-Loan-` という接頭辞のロググループをすべて削除します。
